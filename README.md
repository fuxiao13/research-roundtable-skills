...
- `Lean`、`Standard`、`Thorough` 三档评审强度
- 额度限制是软目标：压缩重复内容，不遗漏实质意见
- Kimi 缓存集中保存，方便定期清理
...
| 模式 | 适用场景 | 输入上限 | 输出策略 |
|---|---|---:|---|
| Lean | 日常迭代 | 16,000 字符 | 约 8 条为软目标 |
| Standard | 重要设计或实验决策 | 24,000 字符 | 约 12 条为软目标 |
| Thorough | 最终或高风险审查 | 24,000 字符 | 保留全部实质意见 |

所有模式都要求保留每一条重要发现；仅压缩复述、重复意见和背景铺垫。
